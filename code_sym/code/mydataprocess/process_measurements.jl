...
include("process_local_measurements.jl")
...
function process_measurements(
    ...
    _process_global_measurements(folder, N_bins, pIDs, β, N_sites, N_start = N_start)

    # process local measurement
    _process_local_measurements(folder, N_bins, pIDs, N_start = N_start)

    # process correlation measurement
    if time_displaced
        _process_correlation_measurements(folder, N_bins, pIDs, ["equal-time", "time-displaced", "integrated"], ["position", "momentum"], Lτ, model_geometry, N_start = N_start)
    else
        _process_correlation_measurements(folder, N_bins, pIDs, ["equal-time", "integrated"], ["position", "momentum"], Lτ, model_geometry, N_start = N_start)
    end
