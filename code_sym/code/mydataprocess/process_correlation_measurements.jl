...
function _process_correlation_measurements(
    ...
    bin_intervals = get_bin_intervals(folder, N_bins, pIDs[1], N_start = N_start)
    @show "correlation bin_intervals", bin_intervals
                ...
                if type == "time-displaced"
                    if haskey(CORRELATION_FUNCTIONS, correlation)
                        _process_correlation_measurement(
                            folder, correlation, space, pIDs, bin_intervals, binned_signs, model_geometry, Lτ, false, N_start = N_start
                        )
                    else
                    ...
                    if haskey(CORRELATION_FUNCTIONS, correlation)
                        _process_correlation_measurement(
                            folder, correlation, type, space, pIDs, bin_intervals, binned_signs, model_geometry, false, N_start = N_start
                        )
                    else
                        _process_composite_correlation_measurement(
                            folder, correlation, type, space, pIDs, bin_intervals, binned_signs, model_geometry, false, N_start = N_start
...
function _process_correlation_measurements(
    ...
    # calculate bin intervals
    bin_intervals = get_bin_intervals(folder, N_bins, pID, N_start = N_start)
        ...
        for correlation in correlations
            ...
            for space in spaces
                ...
                if type == "time-displaced"
                    if haskey(CORRELATION_FUNCTIONS, correlation)
                        _process_correlation_measurement(
                            folder, correlation, space, [pID], bin_intervals, binned_signs, model_geometry, Lτ, true, N_start = N_start
                        )
                    else
                        _process_composite_correlation_measurement(
                            folder, correlation, space, [pID], bin_intervals, binned_signs, model_geometry, Lτ, true, N_start = N_start
                    ...
                    if haskey(CORRELATION_FUNCTIONS, correlation)
                        _process_correlation_measurement(
                            folder, correlation, type, space, [pID], bin_intervals, binned_signs, model_geometry, true, N_start = N_start
                        )
                    else
                        _process_composite_correlation_measurement(
                            folder, correlation, type, space, [pID], bin_intervals, binned_signs, model_geometry, true, N_start = N_start
...
function _process_correlation_measurement(
    ...
    # write_folder = @sprintf "%s/%d_%d_" write_folder N_start N_bins
    ...
    corr = JLD2.load(joinpath(read_folder, @sprintf("bin-1_pID-%d.jld2", pIDs[1])), "correlations")[1]
    
    # correlation container for stats
    binned_correlation = zeros(Complex{T}, N_bins, size(corr)[1:end-1]...)
    correlation_avg = zeros(Complex{T}, size(corr)[1:end-1]...)
    correlation_std = zeros(T, size(corr)[1:end-1]...)
    correlation_var = correlation_std
    ####by kangw####
    
    # stats filename
    if single_pID
        filename = @sprintf("%d_%d_%s_%s_%s_pID-%d_stats.csv", N_start, N_bins, correlation, space, "time-displaced", pIDs[1])
    else
        filename = @sprintf("%d_%d_%s_%s_%s_stats.csv", N_start, N_bins, correlation, space, "time-displaced")
    end
...
function _process_correlation_measurement(
    ...
    # correlation container for stats
    binned_correlation = zeros(Complex{T}, N_bins,size(corr)...)
    correlation_avg = zeros(Complex{T}, size(corr)...)
    correlation_std = zeros(T, size(corr)...)
    correlation_var = correlation_std
    ####by kangw####

    # stats filename
    if single_pID
        filename = @sprintf("%d_%d_%s_%s_%s_pID-%d_stats.csv", N_start, N_bins, correlation, space, type, pIDs[1])
    else
        filename = @sprintf("%d_%d_%s_%s_%s_stats.csv", N_start, N_bins, correlation, space, type)
    end
