...
function _process_local_measurements(folder::String, N_bins::Int, pIDs::Vector{Int}; N_start::Int = 1)

    # calculate bin intervals
    bin_intervals = get_bin_intervals(folder, N_bins, pIDs[1]; N_start = N_start)
    @show "local bin_intervals", bin_intervals
    # get binned sign
    ...
    write_local_measurements((@sprintf "%s/%d_%d_" folder N_start N_bins), local_measurements_avg, local_measurements_std)
...
function write_local_measurements(
    ...
    open((@sprintf "%slocal_stats.csv" folder), "w") do fout
