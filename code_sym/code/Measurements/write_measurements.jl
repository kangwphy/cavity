...
function write_measurements!(;
    ...
    # get hopping and photon to bond ID mappings
    hopping_to_bond_id = measurement_container.hopping_to_bond_id::Vector{Int}
    photon_to_bond_id = measurement_container.photon_to_bond_id::Vector{Int}
    ...
    for correlation in keys(equaltime_correlations)
        ...
        for i in eachindex(correlations)
            ...
            elseif CORRELATION_FUNCTIONS[correlation] == "PHOTON_ID"
                photon_b_id, photon_a_id = pairs[i]
                bond_a_id = photon_to_bond_id[photon_a_id]
                bond_b_id = photon_to_bond_id[photon_b_id]
            end
