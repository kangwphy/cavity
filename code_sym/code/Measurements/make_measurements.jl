...
#######################################################
include("src/myJDQMCMeasurements.jl")
# using JDQMCMeasurements
# export myJDQMCMeasurements
using .myJDQMCMeasurements
...
Make simplified measurements in local updates.
...
"""
function make_static_measurements!(
    measurement_container::NamedTuple,
    logdetGup::E, sgndetGup::T, Gup::AbstractMatrix{T},
    Gup_ττ::AbstractMatrix{T}, Gup_τ0::AbstractMatrix{T}, Gup_0τ::AbstractMatrix{T},
    logdetGdn::E, sgndetGdn::T, Gdn::AbstractMatrix{T},
    ...
    make_equaltime_measurements!(
        ...
        fermion_path_integral_up, fermion_path_integral_dn, l₀
    )
...
Make simplified measurements in loccal updates.
...
function make_static_measurements!(
    ...
    make_equaltime_measurements!(
        ...
        fermion_path_integral, fermion_path_integral, l₀
    )
...
function make_global_measurements!(
    ...
    coupling_parameters::Tuple,
    Gup::AbstractMatrix{T}, logdetGup, sgndetGup::T,
    Gdn::AbstractMatrix{T}, logdetGdn, sgndetGdn::T,
    ####origin
...
function make_local_measurements!(
    ...
    fermion_path_integral_up::FermionPathIntegral{T,E},
    fermion_path_integral_dn::FermionPathIntegral{T,E},
    coupling_parameters::Tuple,l₀::Int64
...
function make_local_measurements!(
    ...
    tight_binding_parameters_dn::TightBindingParameters{T,E},
    fermion_path_integral_up::FermionPathIntegral{T,E},
    fermion_path_integral_dn::FermionPathIntegral{T,E},l₀::Int64
) where {T<:Number, E<:AbstractFloat, D, N}
    ...
    if nhopping > 0
        for n in 1:nhopping
            ...
            hup = sgn * measure_hopping_energy(tight_binding_parameters_up, fermion_path_integral_up, Gup, bond_id, l₀)
            hdn = sgn * measure_hopping_energy(tight_binding_parameters_dn, fermion_path_integral_up, Gdn, bond_id, l₀)
            h = hup + hdn
...
# make local measurements associated with electron-photon model
function make_local_measurements!(
    local_measurements::Dict{String, Vector{Complex{E}}},
    Gup::AbstractMatrix{T}, Gdn::AbstractMatrix{T}, sgn::T,
    model_geometry::ModelGeometry{D,E,N},
    ...
    nphoton = electron_photon_parameters.photon_parameters.nphoton::Int # number of photon modes per unit cell
    nholstein = electron_photon_parameters.holstein_parameters_up.nholstein::Int # number of types of holstein couplings
    nssh = electron_photon_parameters.ssh_parameters_up.nssh::Int # number of types of ssh coupling
    nmin = electron_photon_parameters.min_parameters_up.nmin::Int # number of types of ssh coupling
    ndispersion = electron_photon_parameters.dispersion_parameters.ndispersion::Int # number of types of dispersive photon couplings

    # make photon mode related measurements
    for n in 1:nphoton
        local_measurements["photon_kin_energy"][n]   += sgn * measure_photon_kinetic_energy(electron_photon_parameters, n)
        local_measurements["photon_pot_energy"][n] += sgn * measure_photon_potential_energy(electron_photon_parameters, n)
        local_measurements["X"][n]  += sgn * measure_photon_position_moment(electron_photon_parameters, n, 1)
        local_measurements["X2"][n] += sgn * measure_photon_position_moment(electron_photon_parameters, n, 2)
        local_measurements["X3"][n] += sgn * measure_photon_position_moment(electron_photon_parameters, n, 3)
        local_measurements["X4"][n] += sgn * measure_photon_position_moment(electron_photon_parameters, n, 4)
    end

    # check if finite number of holstein couplings
    if nholstein > 0
        holstein_parameters_up = electron_photon_parameters.holstein_parameters_up
        holstein_parameters_dn = electron_photon_parameters.holstein_parameters_dn
        # make holstein coupling related measurements
        for n in 1:nholstein
            ϵ_hol_up = sgn * measure_holstein_energy(holstein_parameters_up, Gup, x, n)
            ϵ_hol_dn = sgn * measure_holstein_energy(holstein_parameters_dn, Gdn, x, n)
    ...
    if nssh > 0
        # make ssh coupling related measurements
        for n in 1:nssh
            ssh_parameters_up = electron_photon_parameters.ssh_parameters_up
            ssh_parameters_dn = electron_photon_parameters.ssh_parameters_dn
            ϵ_ssh_up = sgn * measure_ssh_energy(ssh_parameters_up, Gup, x, n)
            ϵ_ssh_dn = sgn * measure_ssh_energy(ssh_parameters_dn, Gdn, x, n)
            ϵ_ssh = ϵ_ssh_up + ϵ_ssh_dn
            local_measurements["ssh_energy_up"][n] += ϵ_ssh_up
    ...
    # check if finite number of min couplings
    if nmin > 0
        # make min coupling related measurements
        for n in 1:nmin
            min_parameters_up = electron_photon_parameters.min_parameters_up
            min_parameters_dn = electron_photon_parameters.min_parameters_dn
            ϵ_min_up = sgn * measure_min_energy(min_parameters_up, Gup, x, n)
            ϵ_min_dn = sgn * measure_min_energy(min_parameters_dn, Gdn, x, n)
            ϵ_min = ϵ_min_up + ϵ_min_dn
            local_measurements["min_energy_up"][n] += ϵ_min_up
            local_measurements["min_energy_dn"][n] += ϵ_min_dn
            local_measurements["min_energy"][n] += ϵ_min
        end
    end

    # check if finite number of dispersive photon coupling
    if ndispersion > 0
        # make ssh coupling related measurements
        for n in 1:ndispersion
            local_measurements["dispersion_energy"][n] += sgn * measure_dispersion_energy(electron_photon_parameters, n)
        end
    end
...
function make_equaltime_measurements!(
    ...
    for correlation in keys(equaltime_correlations)
        ...
        average = correlation_container.average

        if correlation == "greens"

            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, Gup_τ0, sgn/2, average)
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, Gdn_τ0, sgn/2, average)
            end
        ...
        elseif correlation == "greens_up"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, Gup_τ0, sgn, average)
            end
        ...
        elseif correlation == "greens_dn"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, Gdn_τ0, sgn, average)
            end

        elseif correlation == "holegreens"

            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, -transpose(Gup_0τ), sgn/2, average)
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, -transpose(Gdn_0τ), sgn/2, average)
            end

        elseif correlation == "holegreens_up"

            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, -transpose(Gup_0τ), sgn, average)
            end

        elseif correlation == "holegreens_dn"

            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                greens!(correlations[i], pair[2], pair[1], unit_cell, lattice, -transpose(Gdn_0τ), sgn, average)
            end
        
        elseif correlation == "greens_onsite"

            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlations[i] += sgn/2 * reshape(diag(Gup_τ0) + diag(Gdn_τ0), Tuple(lattice.L))
            end

        elseif correlation == "holegreens_onsite"

            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlations[i] += sgn/2 *reshape( - diag(Gup_0τ) - diag(Gdn_0τ), Tuple(lattice.L))
            end
        ...
        elseif correlation == "density_upup"
            ...
            for i in eachindex(id_pairs)
                ...
                density_correlation!(correlations[i], pair[2], pair[1], unit_cell, lattice,
                                     Gup_τ0, Gup_0τ, Gup_ττ, Gup, +1, +1, sgn, average)
            end
        ...
        elseif correlation == "density_dndn"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                density_correlation!(correlations[i], pair[2], pair[1], unit_cell, lattice,
                                     Gdn_τ0, Gdn_0τ, Gdn_ττ, Gdn, -1, -1, sgn, average)
            end
        ...
        elseif correlation == "density_updn"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                density_correlation!(correlations[i], pair[2], pair[1], unit_cell, lattice,
                                     Gup_τ0, Gup_0τ, Gup_ττ, Gdn, +1, -1, sgn, average)
            end
        ...
        elseif correlation == "spin_x"
            ...
            for i in eachindex(id_pairs)
                ...
                spin_x_correlation!(correlations[i], pair[2], pair[1], unit_cell, lattice,
                                    Gup_τ0, Gup_0τ, Gdn_τ0, Gdn_0τ, sgn, average)
            end
        ...
        elseif correlation == "current_upup"
            ...
            for i in eachindex(id_pairs)
                ...
                tup0 = @view tup[bond_slices[hopping_id_0], l₀]
                tup0′ = reshape(tup0, lattice.L...)
                tup1 = @view tup[bond_slices[hopping_id_1], l₀]
        ...
        elseif correlation == "current_dndn"
            ...
            for i in eachindex(id_pairs)
                ...
                bond_1 = bonds[bond_id_1]
                # get the effective hopping amptlitudes for each of the two hopping ID's in question
                tdn0 = @view tdn[bond_slices[hopping_id_0], l₀]
                tdn0′ = reshape(tdn0, lattice.L...)
                tdn1 = @view tdn[bond_slices[hopping_id_1], l₀]
    ...
    for correlation in keys(time_displaced_correlations)
        ...
        if correlation == "greens"
            ...
            for i in eachindex(id_pairs)
                ...
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gup_τ0, sgn/2, average)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gdn_τ0, sgn/2, average)
        ...
        elseif correlation == "greens_up"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gup_τ0, sgn, average)
            end
        ...
        elseif correlation == "greens_dn"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gdn_τ0, sgn, average)
            end
        ...
        elseif correlation == "holegreens"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, - transpose(Gup_0τ), sgn/2, average)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, - transpose(Gdn_0τ), sgn/2, average)
            end
        ...
        elseif correlation == "holegreens_up"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, - transpose(Gup_0τ), sgn, average)
            end
        ...
        elseif correlation == "holegreens_dn"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, - transpose(Gdn_0τ), sgn, average)
            end
        ...
        elseif correlation == "greens_onsite"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlations[i][:,:,l+1] += sgn/2 * reshape(diag(Gup_τ0) + diag(Gdn_τ0), Tuple(lattice.L))
            end
        ...
        elseif correlation == "holegreens_onsite"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlations[i][:,:,l+1] += sgn/2 * reshape( - diag(Gup_0τ) - diag(Gdn_0τ), Tuple(lattice.L))
            end
        ...
        elseif correlation == "greens_tautau"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gup_ττ, sgn/2, average)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gdn_ττ, sgn/2, average)
            end
        ...
        elseif correlation == "greens_tautau_up"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gup_ττ, sgn, average)
            end
        ...
        elseif correlation == "greens_tautau_dn"
            ...
            for i in eachindex(id_pairs)
                pair = id_pairs[i]
                correlation_array = selectdim(correlations[i], D+1, l+1)
                greens!(correlation_array, pair[2], pair[1], unit_cell, lattice, Gdn_ττ, sgn, average)
            end
...
########### by kangw ###########
####################################
## MEASURE PHOTON GREENS FUNCTION ##
####################################

# measure equal-time photon greens function
function measure_equaltime_photon_greens!(
    photon_greens::CorrelationContainer{D,E},
    electron_photon_parameters::ElectronPhotonParameters{T,E},
    model_geometry::ModelGeometry{D,E,N},
    sgn::T,
    XrX0::AbstractArray{Complex{E},P},
    ...
    id_pairs = photon_greens.id_pairs::Vector{NTuple{2,Int}}
    correlations = photon_greens.correlations::Vector{Array{Complex{E}, D}}
    lattice = model_geometry.lattice::Lattice{D}
    ...
    # number of photons per unit cell
    photon_parameters = electron_photon_parameters.photon_parameters::PhotonParameters{E}
    nphoton = photon_parameters.nphoton::Int

    # reshape photon field matrix into multi-dimensional array
    x′ = reshape(x, (L..., nphoton, Lτ))

    # iterate over all pairs of photon modes
    for i in eachindex(id_pairs)
        # get the photon fields associated with the appropriate pair of photon modes in the unit cell
        id_pair = id_pairs[i]
        correlation = correlations[i]
        x0 = selectdim(x′, D+1, id_pair[1])
        xr = selectdim(x′, D+1, id_pair[2])
        copyto!(X0, x0)
        copyto!(Xr, xr)
        # calculate photon greens function
        translational_avg!(XrX0, Xr, X0, restore = false)
        # record the equal-time photon green's function
        XrX0_0 = selectdim(XrX0, D+1, 1)
        @. correlation += sgn * XrX0_0
    end
...
# measure equal-time composite photon greens function
function measure_equaltime_composite_photon_greens!(
    photon_greens::CompositeCorrelationContainer{D,E},
    electron_photon_parameters::ElectronPhotonParameters{T,E},
    model_geometry::ModelGeometry{D,E,N},
    sgn::T,
    XrX0::AbstractArray{Complex{E},P},
    ...
    @assert photon_greens.correction == "photon_greens"
    ids = photon_greens.id_pairs::Vector{Int}
    coefficients = photon_greens.coefficients::Vector{Complex{E}}
    correlations = photon_greens.correlations::Array{Complex{E}, D}
    structure_factors = photon_greens.structure_factors::Array{Complex{E}, D}
    lattice = model_geometry.lattice::Lattice{D}
    unit_cell = model_geometry.unit_cell::UnitCell{D,E}
    photon_parameters = electron_photon_parameters.photon_parameters::PhotonParameters{E}
    photon_to_site = photon_parameters.photon_to_site::Vector{Int}
    ...
    for j in eachindex(ids)
        # get orbital species
        photon = N_unitcells * (ids[j]-1) + 1
        sites = photon_to_site(photon)
        orbital_j = site_to_orbital(site, unit_cell)
        for i in eachindex(ids)
            # get orbital species
            photon = N_unitcells * (ids[i]-1) + 1
            sites = photon_to_site(photon)
            orbital_i = site_to_orbital(site, unit_cell)
            # get the photon fields associated with the appropriate pair of photon modes in the unit cell
            x0 = selectdim(x′, D+1, ids[j])
            ...
            # calculate photon greens function
            translational_avg!(XrX0, Xr, X0, restore = false)
            # record the equal-time photon green's function in position space
            XrX0_0 = selectdim(XrX0, D+1, 1)
            coef = conj(coefficients[i]) * coefficients[j]
            @. correlations += sgn * coef * XrX0_0
            # record the equal-time photon green's function in momentum space
            fourier_transform!(XrX0_0, orbital_i, orbital_j, unit_cell, lattice)
            @. structure_factors += sgn * coef * XrX0_0
        end
    end
...
# measure time-displaced photon greens function
function measure_time_displaced_photon_greens!(
    photon_greens::CorrelationContainer{P,E}, # time-displaced because P != D
    electron_photon_parameters::ElectronPhotonParameters{T,E},
    model_geometry::ModelGeometry{D,E,N},
    sgn::T,
    XrX0::AbstractArray{Complex{E},P},
    ...
    id_pairs = photon_greens.id_pairs::Vector{NTuple{2,Int}}
    correlations = photon_greens.correlations::Vector{Array{Complex{E}, P}}
    lattice = model_geometry.lattice::Lattice{D}
    photon_parameters = electron_photon_parameters.photon_parameters::PhotonParameters{E}
...
# measure time-displaced composite photon greens function
function measure_time_displaced_composite_photon_greens!(
    photon_greens::CompositeCorrelationContainer{P,E}, # time-displaced because P != D
    electron_photon_parameters::ElectronPhotonParameters{T,E},
    model_geometry::ModelGeometry{D,E,N},
    sgn::T,
    XrX0::AbstractArray{Complex{E},P},
    ...
    for j in eachindex(ids)
        ...
        for i in eachindex(ids)
            ...
            copyto!(X0, x0)
            copyto!(Xr, xr)
            # calculate photon greens function in position space
            translational_avg!(XrX0, Xr, X0, restore = false)
            correlation′ = selectdim(correlations, D+1, 1:Lτ)
            coef = conj(coefficients[i]) * coefficients[j]
            @. correlation′ += coef * sgn * XrX0
            ...
            correlation_Lτ = selectdim(correlations, D+1, Lτ+1)
            copyto!(correlation_Lτ, correlation_0)
            # calculate photon greens function in position space in momentum space
            fourier_transform!(XrX0, orbital_i, orbital_j, D+1, unit_cell, lattice)
            structure_factors′ = selectdim(structure_factors, D+1, 1:Lτ)
            @. structure_factors′ += coef * sgn * XrX0
            structure_factors_0  = selectdim(structure_factors, D+1, 1)
