...
function initialize_correlation_measurements!(;
    ...
    time_displaced::Bool,
    integrated::Bool = false,
    eqt_average::Bool = true,
    tdp_average::Bool = true,
)  where {T<:AbstractFloat, D, N}
...
function initialize_correlation_measurement!(;
    ...
    pair::NTuple{2,Int},
    time_displaced::Bool,
    integrated::Bool = false, eqt_average::Bool = true, tdp_average::Bool = true)  where {T<:AbstractFloat, D, N}
    ...
    if time_displaced || integrated
        # @show "ts",correlation,time_displaced,integrated
        # add time-displaced and integrated correlation key if not present
        if !haskey(time_displaced_correlations, correlation)
            ## by kangw ###
            # all time displaced correlation use average or the memeory cost is too large
            if tdp_average
                time_displaced_correlations[correlation] = CorrelationContainer(D+1, T, time_displaced,true)
                integrated_correlations[correlation] = CorrelationContainer(D, T, false,true)
            else
                time_displaced_correlations[correlation] = CorrelationContainer(D+1, T, time_displaced, false)
                integrated_correlations[correlation] = CorrelationContainer(D, T, false, false)
            end
        ...
        if tdp_average
            ...
            # add time-dispalced correlation measurement
            push!(time_displaced_correlations[correlation].id_pairs, pair)
            push!(time_displaced_correlations[correlation].correlations, zeros(Complex{T}, prod(L), prod(L), Lτ+1))

            # add integrated correlation measurement
            push!(integrated_correlations[correlation].id_pairs, pair)
            push!(integrated_correlations[correlation].correlations, zeros(Complex{T}, prod(L),prod(L)))
        end
    end
    ...
    if !time_displaced
        ...
        if !haskey(equaltime_correlations, correlation)
            ### by kangw ###
            if eqt_average
                equaltime_correlations[correlation] = CorrelationContainer(D, T, false,true)
            else
                equaltime_correlations[correlation] = CorrelationContainer(D, T, false,false)
            end
        end
        ...
        #####Modified to suit the space resolved case ---- by kangw ####
        if eqt_average
        ...
        # @show correlation,time_displaced,eqt_average,tdp_average,size(equaltime_correlations[correlation].correlations[1])
    end
