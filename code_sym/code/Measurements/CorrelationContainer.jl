...
struct CorrelationContainer{D, T<:AbstractFloat}
    ...
    time_displaced::Bool
    # whether or not the correlation measurement is averaged before saved.
    average::Bool
end

# Initialize and return an empty instance of CorrelationContainer for containing correlation data
# in a D dimensional array.
function CorrelationContainer(D::Int, T::DataType, time_displaced::Bool, average::Bool=true)

    correlation_container = CorrelationContainer(NTuple{2,Int}[], Array{Complex{T},D}[], time_displaced,average)
