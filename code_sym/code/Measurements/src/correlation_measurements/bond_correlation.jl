...
function bond_correlation!(
    ...
    bond_correlation!(BB, b′, b″, unit_cell, lattice, Gup_τ0, Gup_0τ, Gup_ττ, Gup_00, +1, +1, sgn, average)

    # dn-dn bond-bond correlation
    bond_correlation!(BB, b′, b″, unit_cell, lattice, Gdn_τ0, Gdn_0τ, Gdn_ττ, Gdn_00, -1, -1, sgn, average)

    # up-dn bond-bond correlation
    bond_correlation!(BB, b′, b″, unit_cell, lattice, Gup_τ0, Gup_0τ, Gup_ττ, Gdn_00, +1, -1, sgn, average)

    # dn-up bond-bond correlation
    bond_correlation!(BB, b′, b″, unit_cell, lattice, Gdn_τ0, Gdn_0τ, Gdn_ττ, Gup_00, -1, +1, sgn, average)
...
function bond_correlation!(
    ...
    if average
        ...
        # BB(τ,r) = BB(τ,r) + Gσ′(a,i+r+r′,τ|b,i+r,τ)⋅Gσ″(c,i+r″,0|d,i,0)
        mycontract_Grr_G00!(BB, Gσ′_ττ, Gσ″_00, a, b, c, d, r′, z, r″, z, 1, unit_cell, lattice, sgn)

        # BB(τ,r) = BB(τ,r) + Gσ′(a,i+r+r′,τ|b,i+r,τ)⋅Gσ″(d,i,0|c,i+r″,0)
        mycontract_Grr_G00!(BB, Gσ′_ττ, Gσ″_00, a, b, d, c, r′, z, z, r″, 1, unit_cell, lattice, sgn)

        # BB(τ,r) = BB(τ,r) + Gσ′(b,i+r,τ|a,i+r+r′,τ)⋅Gσ″(c,i+r″,0|d,i,0)
        mycontract_Grr_G00!(BB, Gσ′_ττ, Gσ″_00, b, a, c, d, z, r′, r″, z, 1, unit_cell, lattice, sgn)

        # BB(τ,r) = BB(τ,r) + Gσ′(b,i+r,τ|a,i+r+r′,τ)⋅Gσ″(d,i,0|c,i+r″,0)
        mycontract_Grr_G00!(BB, Gσ′_ττ, Gσ″_00, b, a, d, c, z, r′, z, r″, 1, unit_cell, lattice, sgn)

        # if bond correlation between same spin species
        if σ′ == σ″

            # BB(τ,r) = BB(τ,r) - Gσ′(c,i+r″,0|b,i+r,τ)⋅Gσ′(a,i+r+r′,τ|d,i,0)
            mycontract_G0r_Gr0!(BB, Gσ′_0τ, Gσ′_τ0, c, b, a, d, r″, z, r′, z, -1, unit_cell, lattice, sgn)

            # BB(τ,r) = BB(τ,r) - Gσ′(d,i,0|b,i+r,τ)⋅Gσ′(a,i+r+r′,τ|c,i+r″,0)
            mycontract_G0r_Gr0!(BB, Gσ′_0τ, Gσ′_τ0, d, b, a, c, z, z, r′, r″, -1, unit_cell, lattice, sgn)

            # BB(τ,r) = BB(τ,r) - Gσ′(c,i+r″,0|a,i+r+r′,τ)⋅Gσ′(b,i+r,τ|d,i,0)
            mycontract_G0r_Gr0!(BB, Gσ′_0τ, Gσ′_τ0, c, a, b, d, r″, r′, z, z, -1, unit_cell, lattice, sgn)

            # BB(τ,r) = BB(τ,r) - Gσ′(d,i,0|a,i+r+r′,τ)⋅Gσ′(b,i+r,τ|c,i+r″,0)
            mycontract_G0r_Gr0!(BB, Gσ′_0τ, Gσ′_τ0, d, a, b, c, z, r′, z, r″, -1, unit_cell, lattice, sgn)
        end
    end
