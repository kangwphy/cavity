...
- `neighbor_table::Matrix{Int}`: Neighbor table where the first row specifies the site where the photon mode is located, and the second row specifies the site corresponding to the density getting coupled to.
...
struct HolsteinParameters{E<:AbstractFloat}
    ...
    # neighbor table for couplings where first row is the site the photon the lives on,
    # and the second row is the site whose density the photon mode is coupling to
