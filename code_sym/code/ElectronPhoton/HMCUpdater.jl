...
Defines a hybrid/hamiltonian monte carlo (HMC) update for the photon degrees of freedom.
...
- `dSfdx0::Matrix{E}`: Initial derivative of fermionic action associated with the initial photon configuration.
...
- `x′::Matrix{E}`: Array to record intermediate photon configurations.
- `x0::Matrix{E}`: Array to record initial photon configuration.
- `v::Matrix{E}`: Conjugate momentum to photon fields in HMC trajectory.
...
mutable struct HMCUpdater{T<:Number, E<:AbstractFloat, PFFT, PIFFT}
    ...
    # contains intermediate photon configuration
    const x′::Matrix{E}

    # record initial photon configuration at the start of the trajectory
    const x0::Matrix{E}
...
function HMCUpdater(;
    ...
    M = FourierMassMatrix(electron_photon_parameters, reg)
...
Perform HMC update to the photon degrees of freedom.
...
Perform HMC update to the photon degrees of freedom assuming the spin-up and spin-down sectors are equivalent.
...
function hmc_update!(
    ...
    (accepted, logdetG, sgndetG, δG, δθ) = _hmc_update!(
        G, logdetG, sgndetG, Gup′, electron_photon_parameters,
