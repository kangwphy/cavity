...
hybrid/hamiltonian monte carlo or langevin monte carlo updates to the photon fields.
...
@doc raw"""
    FourierMassMatrix(electron_photon_parameters::ElectronPhotonParameters{T,E}, reg::E=1.0) where {T,E}
...
Also, ``\Omega`` and ``M`` are the photon frequency and mass respectively.
"""
function FourierMassMatrix(electron_photon_parameters::ElectronPhotonParameters{T,E}, reg::E=1.0) where {T,E}
    ...
    M = photon_parameters.M::Vector{E} # photon mass
    Ω = photon_parameters.Ω::Vector{E} # photon frequency
    ...
    for ω in axes(M̃, 2)
        # iterate over photon fields
        for n in axes(M̃, 1)
            # if inifinite photon mass
            if isinf(M[n])
            ...
            elseif isinf(reg)
                ...
                M̃[n, ω] = M[n] * Δτ
            # if finite photon mass and regularization
            else
                # CHANGE IN FUTURE: if bare on-site photon frequency is zero, default to 1.0
...
# The rows of `v` correspond to photon modes, and the columns correspond to imaginary time slices.
...
# The rows of `Mv` and `v` correspond to photon modes, and the columns correspond to imaginary time slices.
