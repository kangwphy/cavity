# performs a hybrid/hamiltonian monte carlo update to the photon configuration
function _hmc_update!(Gup::Matrix{T}, logdetGup::E, sgndetGup::T, Gup′::Matrix{T},
    ...
    (; β, Lτ, Δτ) = electron_photon_parameters
    ...
    calculate_exp_K = (ssh_parameters_up.Nssh > 0) || (min_parameters_up.Nmin > 0)
    ...
    for t in 1:Nt
        ...
        fill!(dSdx, 0.0)
        bosonic_action_derivative!(dSdx, electron_photon_parameters, holstein_correction=false)
        ...
        for t′ in 1:nt
            ...
            # re-center photon fields assuming translationally invariant hamiltonian.
        ...
        # update the fermion path integrals for spin up and down sectors to reflect current photon configuration
        update!(fermion_path_integral_up, fermion_path_integral_dn, electron_photon_parameters, x, x′)
        
        # record the current photon configuration
        copyto!(x′, x)
    ...
    # if proposed photon configuration is accepted and hmc trajecotry remained numerically stable
    if accepted
    ...
    # otherwise reject proposed update and revert to oringial photon configuration
    else
        # update fermion path integrals to reflect initial photon configuration
        update!(fermion_path_integral_up, fermion_path_integral_dn, electron_photon_parameters, x0, x)
        # revert to initial photon configuration
        copyto!(x, x0)
        # update propagators to reflect initial photon configuration
        calculate_propagators!(Bup, fermion_path_integral_up,
                               calculate_exp_K = calculate_exp_K, calculate_exp_V = calculate_exp_V)
        calculate_propagators!(Bdn, fermion_path_integral_dn,
                               calculate_exp_K = calculate_exp_K, calculate_exp_V = calculate_exp_V)
...
# performs a hybrid/hamiltonian monte carlo update to the photon configuration assuming
...
function  _hmc_update!(G::Matrix{T}, logdetG::E, sgndetG::T, G′::Matrix{T},
    ...
    if initialize_force || first_update
        ...
        # calculate derivative associated with Holstein correction
        eval_derivative_holstein_action!(dSdx, x, Δτ, holstein_parameters, photon_parameters) # spin-up
        eval_derivative_holstein_action!(dSdx, x, Δτ, holstein_parameters, photon_parameters) # spin-down
    ...
    for t in 1:Nt
        ...
        # update the fermion path integrals for spin up and down sectors to reflect current photon configuration
        update!(fermion_path_integral, electron_photon_parameters, x, x′)
    ...
    # otherwise reject proposed update and revert to oringial photon configuration
        ...
        # update fermion path integrals to reflect initial photon configuration
        update!(fermion_path_integral, electron_photon_parameters, x0, x)
        # revert to initial photon configuration
        copyto!(x, x0)
        # update propagators to reflect initial photon configuration
        calculate_propagators!(B, fermion_path_integral, calculate_exp_K = calculate_exp_K, calculate_exp_V = calculate_exp_V)
