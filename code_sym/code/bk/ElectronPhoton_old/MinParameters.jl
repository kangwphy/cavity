    ...
    if true
        ...
        #     # get the min coupling definition
            ...
            for ith in 1:size(min_neighbor_table,2)
                ...
                min_counter += 1
                y = div(min_neighbor_table[1,ith]-1,lattice.L[1])


                α_up[min_counter]  = y*min_coupling_up.α_mean  + min_coupling_up.α_std  * randn(rng)
                α2_up[min_counter] = y*min_coupling_up.α2_mean + min_coupling_up.α2_std * randn(rng)
                α3_up[min_counter] = y*min_coupling_up.α3_mean + min_coupling_up.α3_std * randn(rng)
                α4_up[min_counter] = y*min_coupling_up.α4_mean + min_coupling_up.α4_std * randn(rng)
    
                α_dn[min_counter]  = y*min_coupling_dn.α_mean  + min_coupling_dn.α_std  * randn(rng)
                α2_dn[min_counter] = y*min_coupling_dn.α2_mean + min_coupling_dn.α2_std * randn(rng)
                α3_dn[min_counter] = y*min_coupling_dn.α3_mean + min_coupling_dn.α3_std * randn(rng)
                α4_dn[min_counter] = y*min_coupling_dn.α4_mean + min_coupling_dn.α4_std * randn(rng)
                tc[min_counter] = 1
            end
            # @show PBCx
            if PBCx != 1
