...
function run_photon_minicoup_square_simulation(sID, U, Ω, g, μ, β, Lx, Ly, PBCx, PBCy, N_burnin, N_updates, N_bins, eqt_average, tdp_average; filepath = ".")
...
#     #################################
   ...
   # Synchronize all the MPI processes.
    ...
    # completion.
    MPI.Barrier(comm)
    # t = measurement_container.equaltime_correlations["current"]
    # @show t
    # @show size(t)
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    prefix = ARGS[16]
    ## Run the simulation.
    run_photon_minicoup_square_simulation(sID, U, Ω, g, μ, β, Lx, Ly, PBCx, PBCy, N_burnin, N_updates, N_bins, eqt_average, tdp_average; filepath = "./data/"*prefix)
end
