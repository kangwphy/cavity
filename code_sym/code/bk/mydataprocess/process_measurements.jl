...
function process_measurements(
    ...
    _process_global_measurements(folder, N_bins, N_start, pIDs, β, N_sites)

    # process local measurement
    _process_local_measurements(folder, N_bins, N_start, pIDs)

    # process correlation measurement
    if time_displaced
        _process_correlation_measurements(folder, N_bins, N_start, pIDs, ["equal-time", "time-displaced", "integrated"], ["position", "momentum"], Lτ, model_geometry)
    else
        _process_correlation_measurements(folder, N_bins, N_start, pIDs, ["equal-time", "integrated"], ["position", "momentum"], Lτ, model_geometry)
    end
