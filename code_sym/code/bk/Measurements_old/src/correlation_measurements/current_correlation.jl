...
include("../myutilities.jl")
# include("../JDQMCMeasurements.jl")
...
function current_correlation!(
    ...
    current_correlation!(CC, b′, b″, tup′, tup″, unit_cell, lattice, Gup_τ0, Gup_0τ, Gup_ττ, Gup_00, +1, +1, sgn, translation)

    # dn-dn current-current correlation
    current_correlation!(CC, b′, b″, tdn′, tdn″, unit_cell, lattice, Gdn_τ0, Gdn_0τ, Gdn_ττ, Gdn_00, -1, -1, sgn, translation)

    # up-dn current-current correlation
    current_correlation!(CC, b′, b″, tup′, tdn″, unit_cell, lattice, Gup_τ0, Gup_0τ, Gup_ττ, Gdn_00, +1, -1, sgn, translation)

    # dn-up current-current correlation
    current_correlation!(CC, b′, b″, tdn′, tup″, unit_cell, lattice, Gdn_τ0, Gdn_0τ, Gdn_ττ, Gup_00, -1, +1, sgn, translation)
...
function current_correlation!(
    ...
    σ′::Int, σ″::Int, sgn::T=one(T), translation::Bool=true
) where {D, C<:Number, T<:Number}
    ...
    if translation
        ...
        # CC(τ,r) = CC(τ,r) + sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(a,i+r+r′,τ|b,i+r,τ)⋅Gσ″(d,i,0|c,i+r″,0)
        mycontract_Grr_G00!(CC, Gσ′_ττ, Gσ″_00, t′, t″, a, b, d, c, r′, z, z, r″, +1, unit_cell, lattice, sgn, 2,3)

        # CC(τ,r) = CC(τ,r) - sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(a,i+r+r′,τ|b,i+r,τ)⋅Gσ″(c,i+r″,0|d,i,0)
        mycontract_Grr_G00!(CC, Gσ′_ττ, Gσ″_00, t′, t″, a, b, c, d, r′, z, r″, z, -1, unit_cell, lattice, sgn, 2,4)

        # CC(τ,r) = CC(τ,r) + sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(b,i+r,τ|a,i+r+r′,τ)⋅Gσ″(c,i+r″,0|d,i,0)
        mycontract_Grr_G00!(CC, Gσ′_ττ, Gσ″_00, t′, t″, b, a, c, d, z, r′, r″, z, +1, unit_cell, lattice, sgn, 1,4)

        # CC(τ,r) = CC(τ,r) - sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(b,i+r,τ|a,i+r+r′,τ)⋅Gσ″(d,i,0|c,i+r″,0)
        mycontract_Grr_G00!(CC, Gσ′_ττ, Gσ″_00, t′, t″, b, a, d, c, z, r′, z, r″, -1, unit_cell, lattice, sgn, 1,3)

        # if current correlation between same spin species
        if σ′ == σ″

            # CC(τ,r) = CC(τ,r) + sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(c,i+r″,0|b,i+r,τ)⋅Gσ″(a,i+r+r′,τ|d,i,0)
            mycontract_G0r_Gr0!(CC, Gσ′_0τ, Gσ′_τ0, t′, t″, c, b, a, d, r″, z, r′, z, +1, unit_cell, lattice, sgn, 2,4)

            # CC(τ,r) = CC(τ,r) - sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(d,i,0|b,i+r,τ)⋅Gσ″(a,i+r+r′,τ|c,i+r″,0)
            mycontract_G0r_Gr0!(CC, Gσ′_0τ, Gσ′_τ0, t′, t″, d, b, a, c, z, z, r′, r″, -1, unit_cell, lattice, sgn, 2,1)

            # CC(τ,r) = CC(τ,r) - sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(c,i+r″,0|a,i+r+r′,τ)⋅Gσ″(b,i+r,τ|d,i,0)
            mycontract_G0r_Gr0!(CC, Gσ′_0τ, Gσ′_τ0, t′, t″, c, a, b, d, r″, r′, z, z, -1, unit_cell, lattice, sgn, 3,4)

            # CC(τ,r) = CC(τ,r) + sum_i tσ′(a,i+r|b,i)⋅tσ″(c,i+r″|d,i)⋅Gσ′(d,i,0|a,i+r+r′,τ)⋅Gσ″(b,i+r,τ|c,i+r″,0)
            mycontract_G0r_Gr0!(CC, Gσ′_0τ, Gσ′_τ0, t′, t″, d, a, b, c, z, r′, z, r″, +1, unit_cell, lattice, sgn, 3,1)
        end
