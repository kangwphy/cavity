    ...
    for correlation in keys(equaltime_correlations)
        ...
        elseif correlation == "density"
            ...
            for i in eachindex(id_pairs)
                ...
                correlation = correlations[i]
                translation = false
                @show "density",correlation
                density_correlation!(correlation, pair[2], pair[1], unit_cell, lattice,
                                     Gup_τ0, Gup_0τ, Gup_ττ, Gup, Gdn_τ0, Gdn_0τ, Gdn_ττ, Gdn, sgn,translation)
            end
...
# measure equal-time photon greens function
function measure_equaltime_photon_greens!(photon_greens::CorrelationContainer{D,E},
    electron_photon_parameters::ElectronPhotonParameters{T,E},
    model_geometry::ModelGeometry{D,E,N},
    sgn::T,
    XrX0::AbstractArray{Complex{E},P},
...
bond_id_pairs = photon_greens.bond_id_pairs::Vector{NTuple{2,Int}}
...
# get the site associated with each photon field
photon_to_site = reshape(photon_parameters.photon_to_site, (lattice.N, nphoton))
...
# record the bond id pair if not already recorded
if (bond_id_pairs[i][1] == 0) && (bond_id_pairs[i][2] == 0)
# get the site ID each photon mode lives on
site_1 = photon_to_site[1, id_pair[1]]
site_2 = photon_to_site[1, id_pair[2]]
# get orbital id associated with site
orbital_1 = site_to_orbital(site_1, unit_cell)
orbital_2 = site_to_orbital(site_2, unit_cell)
# record orbital/bond id pair
...
# measure time-displaced photon greens function
function measure_time_displaced_photon_greens!(photon_greens::CorrelationContainer{P,E}, # time-displaced because P != D
         electron_photon_parameters::ElectronPhotonParameters{T,E},
         model_geometry::ModelGeometry{D,E,N},
         sgn::T,
         XrX0::AbstractArray{Complex{E},P},
...
# get the site associated with each photon field
photon_to_site = reshape(photon_parameters.photon_to_site, (1, nphoton))
# @show id_pairs,eachindex(id_pairs),photon_to_site
# iterate over all pairs of photon modes
for i in eachindex(id_pairs)
# get the photon fields associated with the appropriate pair of photon modes in the unit cell
id_pair = id_pairs[i]
...
correlation_Lτ = selectdim(correlation, D+1, Lτ+1)
copyto!(correlation_Lτ, correlation_0)
# @show x0,xr,correlation′,correlation_0,correlation_Lτ
# record the bond id pair if not already recorded
if (bond_id_pairs[i][1] == 0) && (bond_id_pairs[i][2] == 0)
# get the site ID each photon mode lives on
site_1 = photon_to_site[1, id_pair[1]]
