...
function run_photon_minicoup_square_simulation(sID, U, Ω, g, μ, β, Lx, Ly, PBCx, PBCy, N_burnin, N_updates, N_bins; filepath = ".")
    ...
    datafolder_prefix =  @sprintf "%s/photon_minicoup_square_U%.2f_w%.2f_g%.2f_mu%.2f_Lx%d_Ly%d_BC%d%d_b%.2f" dir U Ω g μ Lx Ly PBCx PBCy β
...
#md     ## in order to reconstruct the extended s-wave and d-wave pair susceptibilities.
    ...
    for bin in n_bin_burnin:N_bins_burnin
        ...
        for n in 1:bin_size
            ...
            if iszero(simulation_info.pID) && rem(n,10)==0
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    prefix = ARGS[14]
    ## Run the simulation.
    run_photon_minicoup_square_simulation(sID, U, Ω, g, μ, β, Lx, Ly, PBCx, PBCy, N_burnin, N_updates, N_bins; filepath = "./data/"*prefix)
end
